<template lang="html">
  <div class="login">
    <v-header>
      <h1 slot="title">提现密码</h1>
    </v-header>
    <div style="padding: 0 3vw;"  v-if="re">
      <mt-field label="提现密码：" v-model="reData.password" :readonly='!toggle' type="tel"
                :disableClear='!toggle' placeholder="请输入6位提现密码" :attr="{ maxlength: 6 }">
      </mt-field>
      <mt-field label="手机：" v-model="reData.accountName" :readonly='true'
    ...
    <section style="margin-top: 20px;text-align: center">
      <mt-button style='height:30px;width: 70%;margin: auto' @click="login" type="primary" size="large">确定</mt-button>
    </section>
  </div>
</template>
  ...
  export default {
    ...
    data() {
      return {
        re: true,
        reData: {
          accountName: this.Local.getLocal('user').mobile,
          authCode: '',
          password: ''
        },
    ...
    methods: {
      ...
      login() {
        if (this.reData.accountName !== "" && this.reData.authCode) {
          if (this.reData.accountName !== '') {
            ...
            this.$api({
              method: 'post',
              url: '/api/v1/user/editTransferPass',
              data: data
            }).then((response) => {
      ...
      getCode() {
        ...
        if (this.reData.accountName !== ''&&this.string === '立即获取') {
          ...
          this.$api({
            method: 'get',
            url: `/api/v1/user/code?type=5&debug=false&m=` + this.reData.accountName
          }).then((response) => {
            if(response.data.code === 1000){
              var i = 59;
